...
campsiteRouter.route('/').get((req, res, next) => {
...
campsiteRouter.route('/:campsiteId/comments').get((req, res, next) => {
    Campsite.findById(req.params.campsiteId).populate('comments.author').then(campsite => {
        if (campsite) {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
...
campsiteRouter.route('/:campsiteId/comments/:commentId').get((req, res, next) => {
    ...
    Campsite.findById(req.params.campsiteId).then(campsite => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
            ...
            if (commentAuthorId.equals(requestingUserId)) {
                if (req.body.rating) {
            ...
            } else if (!commentAuthorId.equals(requestingUserId)) {
                err = new Error(`You are not authorized to do that!`);
                err.status = 403;
                return next(err);
            }
        } else if (!campsite) {
    ...
    Campsite.findById(req.params.campsiteId).then(campsite => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
            ...
            err = new Error(`You are not authorized to delete that comment!`);
            err.status = 403;
            return next(err);
            }
        } else if (!campsite) {
